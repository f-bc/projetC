...
int alerte(float T, float val_seuil, int *nb_c, int *nb_m, int *nb_s,char msg_f[]) {
    ...
    char tmp_str[50];
    if (k < 5) {
        snprintf(tmp_str, sizeof(tmp_str), "il y a un depassement leger du temperature seuil de %.2f C\n",k);
        strcat(msg_f, tmp_str);
        (*nb_s)++;   
        return 1;
    }else if (k>=5 && k<= 10) {
        snprintf(tmp_str, sizeof(tmp_str), "il y a un depassement important du temperature seuil de %.2f C!\n",k);
        strcat(msg_f, tmp_str);
        (*nb_m)++;   
        return 2;
    }else if(k>10) { 
        snprintf(tmp_str, sizeof(tmp_str), "danger immediat:depassement du temperature seuil de %.2f C!!\n",k);
        strcat(msg_f, tmp_str);
        (*nb_c)++;   
        return 3;
    }
